...
键盘映射功能允许用户通过键盘按键代替点击屏幕按钮来控制六个方向轴，同时完整保留原有鼠标点击逻辑不变。

#### 设计思路

- 键位绑定关系定义在外部配置文件 `MC104/Configs/keyboard_mapping.json` 中，以 JSON 格式描述每个 action 与对应键位的映射关系，便于在不修改代码的情况下重新分配键位。
- 所有键盘事件都在窗体层面拦截，不依赖任何具体子控件的焦点状态。
- 键盘触发路径和鼠标触发路径最终调用完全相同的 `buttons_MouseDown` / `buttons_MouseUp`，保证行为一致。

#### 配置文件格式

`keyboard_mapping.json` 的结构如下：
...
`action` 字段与 `GetButtonForAction()` 中的 switch-case 对应，`key` 字段使用 `System.Windows.Forms.Keys` 枚举名称，大小写不敏感。

#### 落实方式

1. **配置加载**：`LoadKeyboardMapping()` 在构造函数中被调用，通过 `KeyboardMappingConfig.Load()` 将配置文件反序列化为 `KeyboardMappingConfig` 对象，存入 `keyboardMappingConfig` 字段。若文件缺失或格式错误，弹出警告后以空配置降级运行，不影响其他功能。

2. **表单级按键拦截**：在 `Form2_Load()` 中设置 `KeyPreview = true`，使窗体在子控件之前收到键盘事件，然后注册 `Form_KeyDown` 和 `Form_KeyUp`。

3. **按键抑制**：对匹配到的键位设置 `e.SuppressKeyPress = true`，确保按键字符不会被输入到当前获得焦点的子控件（例如 `stepDistance` 文本框），从而避免输入内容被破坏导致 `double.Parse` 失败报错。

4. **防重复触发**：`heldKeys`（`HashSet<Keys>`）记录当前物理按下但尚未释放的键位。OS 在长按时会持续重复触发 `KeyDown`，`heldKeys` 确保每次物理按下只执行一次动作触发，松开键位时从集合中移除。

5. **Jog / Step 模式适配**：
   - **Jog 模式**：`Form_KeyDown` 调用 `buttons_MouseDown`（开始连续运动），`Form_KeyUp` 调用 `buttons_MouseUp`（停止运动），与鼠标按住/松开的行为完全一致。
   - **Step 模式**：`Form_KeyDown` 同样调用 `buttons_MouseDown` 触发一次步进，由于 step 模式下 `buttons_MouseUp` 本身是空操作，`Form_KeyUp` 调用它不会产生任何副作用。结合 `heldKeys` 防重复，长按键盘时效果与短按相同，即只执行一步。

6. **键位映射查看**：Menu Strip 中的 `Keyboard Mapping` 菜单项点击后会弹出 `KeyboardMappingForm`，以表格形式展示当前配置文件中的全部键位绑定，方便用户查阅。
...
| `LoadKeyboardMapping()` | 读取键盘映射配置文件，失败时以空配置降级。 |
| `keyboardMappingToolStripMenuItem_Click(...)` | 弹出键位映射查看窗口。 |
| `GetButtonForAction(string action)` | 根据 action 名称返回对应的方向控制按钮。 |
| `Form_KeyDown(...)` | 拦截键盘按下事件，防重复后调用 `buttons_MouseDown`。 |
| `Form_KeyUp(...)` | 拦截键盘松开事件，清理 `heldKeys` 并调用 `buttons_MouseUp`。 |

										...
										LPSTR lpCmdLine, int nCmdShow )
{
	...
	wsprintf( szBuf,
		_T("%s\n")
		_T("制御軸 %d ＋方向に 10000 パルス出力しています"), szBoadName, MCUSB4sd_AXIS1 );
	ResultMessage( szBuf );
	///////////////////////////////////////////////////////////////////////////
...
static BOOL Mcusb4sdStatusPrint( HANDLE hDevice, WORD wAxis )
{
	...
	wsprintf( szBuf,
		_T("%s\n")
		_T("制御軸 %d エンドステータス: %Xh"), szBoadName, wAxis, Status.bEnd);
	ResultMessage( szBuf );
	wsprintf( szBuf,
		_T("%s\n")
		_T("制御軸 %d メカニカルシグナル: %Xh"), szBoadName, wAxis, Status.bMechanical);
	ResultMessage( szBuf );
	wsprintf( szBuf,
		_T("%s\n")
		_T("制御軸 %d ユニバーサルシグナル: %Xh"), szBoadName, wAxis, Status.bUniversal);
	ResultMessage( szBuf );
	// ドライブ中であれば減速停止します。

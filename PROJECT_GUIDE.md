...
它不是对现有 [MC104/README.md](MC104/README.md) 的替代，而是对现有 API 说明文档的补充：

- [MC104/README.md](MC104/README.md) 主要描述 TCP 通信协议和外部接口。
- 本文档重点解释整个软件系统如何组织、如何运行、各层之间如何协作，以及关键方法各自承担什么职责。

本文档明确忽略以下内容，因为它们已废弃或不属于当前主运行路径：
...
本项目是一个运行在 Windows 上的 WinForms 三轴微操作器控制程序，用于控制 Microsupport 控制器。软件能够同时连接最多 4 台控制器，并对指定控制器执行基础运动指令，同时扩展了一套 TCP 通信层，用于向外部路径规划程序提供状态查询、绝对步进、路径载入、PTP 轨迹执行和 CP 近似连续轨迹执行能力。
...
|-- driver_manual.md              # 参考手册
|-- controller_mapping.json       # 根目录历史/辅助副本
`-- MC104.sln                     # 解决方案文件
```
...
### 9.4 根目录中的 `controller_mapping.json`

根目录也存在一份 `controller_mapping.json`，但主程序在运行时读取的是：

- `MC104/Configs/controller_mapping.json`

因此根目录这份更像历史副本、参考配置或人工维护文件，而不是当前主程序的直接运行配置。
